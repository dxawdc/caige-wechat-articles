"""v1.0.2 | 独立检查保存后的数据与文件，不重用绘图中的断言结果。"""
...
mapping = json.loads(
    (ROOT / "案例索引_v1.0.2.json").read_text(
        encoding="utf-8"
    )
)
check(
...
for e in mapping:
    n = e["id"]
    code = ROOT / "cases" / f"case_{n:02d}_v1_0_2.py"
    ast.parse(code.read_text(encoding="utf-8"))
    png = OUT / f"{n:02d}_v1.0.2.png"
    ...
    check(
        f"{n:02d}有SVG",
        (OUT / f"{n:02d}_v1.0.2.svg").stat().st_size > 1000,
    )
    record = json.loads(
...
check(
    "交互文件数为7", len(list(OUT.glob("*_v1.0.2.html"))) == 7
)
check(
    "堆叠收入四个合计",
...
(ROOT / "验收报告_v1.0.2.json").write_text(
