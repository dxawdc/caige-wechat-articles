"""v1.2.1 | 2026-09-15 | Markdown转微信内联HTML：语法高亮、逐行代码、紧邻图注。
用法：python 微信排版_v1.2.1.py --input 文章.md --output 输出目录
...
VERSION='v1.2.1'
TITLE='用Python复盘我的S44排位：332场，从钻石到荣耀王者'
URL='https://github.com/dxawdc/caige-wechat-articles/tree/main/articles/2026-09-14-s44-ranked-review'
...
def highlighted_lines(source,language):
    cls=PythonLexer if language in ('python','py') else BashLexer if language in ('bash','sh','shell') else TextLexer
...
def main():
    ...
    for pre in doc.xpath('.//pre'):
        ...
        pre.clear();pre.set('class','code-snippet_nowrap')
        pre.set('style',f'display:block;font-family:{FONT};font-size:11px!important;line-height:18px;max-width:100%;box-sizing:border-box;overflow-x:auto;white-space:pre;word-break:normal;overflow-wrap:normal;tab-size:4;margin:0;padding:12px;background:#152C3D;color:{COLORS["plain"]};border-radius:6px;')
        for row in highlighted_lines(source,language):
            line=etree.SubElement(pre,'code');line.set('style',f'display:block;min-height:18px;font-family:{FONT};font-size:11px!important;line-height:18px;white-space:pre;word-break:normal;overflow-wrap:normal;')
            for color,value in row:
                span=etree.SubElement(line,'span');span.set('style',f'font-family:{FONT};font-size:11px!important;line-height:18px;color:{color}!important;white-space:pre;')
                span.text=value.replace(' ','\xa0')
        outer=etree.Element('section');outer.set('class','code-snippet__fix')
        outer.set('style','display:block;width:100%;max-width:100%;margin:20px 0;box-sizing:border-box;')
