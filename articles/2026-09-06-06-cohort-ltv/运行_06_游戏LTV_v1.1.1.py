"""v1.1.1 | 2026-09-06 | 游戏LTV怎么算：先把分母和观察天数对齐：专属入口，保留包内原始材料。"""
from pathlib import Path
import json,shutil,subprocess,sys
BASE=Path(__file__).resolve().parent
manifest=json.loads((BASE/'物料清单_v1.1.1.json').read_text(encoding='utf-8'))
assert manifest['articleId']=='06-cohort-ltv'
OUT=(BASE/'运行结果'/'06-cohort-ltv').resolve()
...
print('游戏LTV怎么算：先把分母和观察天数对齐：基础与进阶案例运行通过。')
print('结果目录：'+str(OUT))
