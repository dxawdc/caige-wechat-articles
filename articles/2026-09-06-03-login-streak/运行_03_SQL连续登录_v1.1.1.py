"""v1.1.1 | 2026-09-06 | SQL算连续登录：去重、断档和跨月怎么处理：专属入口，保留包内原始材料。"""
from pathlib import Path
import json,shutil,subprocess,sys
BASE=Path(__file__).resolve().parent
manifest=json.loads((BASE/'物料清单_v1.1.1.json').read_text(encoding='utf-8'))
assert manifest['articleId']=='03-login-streak'
OUT=(BASE/'运行结果'/'03-login-streak').resolve()
...
print('SQL算连续登录：去重、断档和跨月怎么处理：基础与进阶案例运行通过。')
print('结果目录：'+str(OUT))
